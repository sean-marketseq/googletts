...
interface BatchDiagnostics {
  ...
  hume_job?: {
    id: string;
    status: string | { state?: string; status?: string; error?: string };
    message?: string;
  };
}
...
function App() {
  ...
  const processNextFile = async (fileId: string) => {
    ...
    try {
      ...
      const errorDetail = error.response?.data?.detail || error.message || 'Failed to upload audio file';
      const errorStatus = error.response?.status ? `[${error.response.status}]` : '';
      const fullError = errorStatus ? `${errorStatus} ${errorDetail}` : errorDetail;
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div className="flex-1 min-w-0">
                            ...
                            {fs.error && (
                              <div className="mt-2 p-2 bg-red-900/30 border border-red-500/50 rounded text-xs text-red-200 break-words">
                                <span className="font-semibold">Error:</span> {fs.error}
                              </div>
                            )}
                          </div>
              ...
              <div>
                ...
                <div className="mb-4 p-4 bg-white/5 border border-purple-400/30 rounded-xl">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <button
                      type="button"
                      onClick={() => setQuery("Analyze calls that resulted in transfers to human agents. Structure your response to START with actionable insights:\n\n## PART 1: EXECUTIVE SUMMARY (Start Here)\n- Top 3-5 themes causing transfers (with frequency counts)\n- Most critical issues (priority: high/medium/low based on emotion data and frequency)\n- **Detailed Hume AI Emotion Patterns by Theme**: For EACH theme, cite the specific Hume AI emotions detected (e.g., Contempt, Determination, Anger, Frustration, Anxiety). Include:\n  * Which emotions appear most frequently with this theme\n  * The emotional progression (how emotions change during the interaction)\n  * Specific emotion scores where available\n  * Speaker-specific emotions (CALLER vs AGENT emotions separately)\n  * Example: \"Direct-to-human theme shows high Contempt (0.82) and Determination (0.74) from CALLER in conversation_id_12345678901234567890abcdef, paired with Calmness (0.65) from AGENT\"\n- Overall sentiment trends\n\n## PART 2: THEME-BASED RECOMMENDATIONS\nFor each theme:\n- Specific system prompt improvements (exact wording to add)\n- Data to collect before transfer\n- **Emotion-based handling with Hume AI**: Provide detailed guidance:\n  * If Hume detects [specific emotion like Contempt, Frustration, Anxiety] above threshold X, then do Y\n  * Emotional escalation patterns to watch for (e.g., Frustration → Anger → Contempt)\n  * Proactive interventions based on emotion detection\n\n## PART 3: DETAILED EMOTION ANALYSIS (Critical - Cite Deeply)\nFor each conversation referenced:\n- **COMPLETE conversation ID** (NEVER use ellipsis, dots, or abbreviations like \"conv_4401...\" - ALWAYS write the FULL ID like \"conv_7001k892nvqyf0hvbgwq3ysyf5fx\")\n- **Hume AI Emotion Deep Dive**: List the top 5-7 emotions detected for each speaker with context:\n  * CALLER emotions: [Emotion1 (score), Emotion2 (score), ...] - what this reveals about customer state\n  * AGENT emotions: [Emotion1 (score), Emotion2 (score), ...] - what this reveals about agent handling\n  * Emotional interplay: How caller and agent emotions interact and influence each other\n  * Emotional turning points: When did emotions shift and why\n- Individual questions/issues that led to transfer\n- Timestamp of transfer if available\n\n## CRITICAL FORMATTING RULES:\n1. **NEVER abbreviate conversation IDs** - Always write complete IDs like \"conv_7001k892nvqyf0hvbgwq3ysyf5fx\"\n2. **NO ellipsis in IDs** - Do not use \"conv_4401...\" or \"Conv_4401…\" - write the FULL ID\n3. **Hume AI emotions are the heart of this analysis** - Cite them extensively with specific emotion names and scores\n4. **Name emotions explicitly** - Use exact Hume AI emotion names (Contempt, Determination, Calmness, Frustration, Anxiety, etc.)\n5. **Provide emotional context** - Explain what each emotion pattern means for the interaction")}
